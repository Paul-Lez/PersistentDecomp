...
def pretty_file(file: str):
    ...
    url = f"https://github.com/Paul-Lez/PersistentDecomp/blob/master/{file}"
    return f"[{name}]({url})"

files = [file for tree in os.walk("PersistentDecomp") for file in glob(os.path.join(tree[0], '*.lean'))]
sorries = {}
