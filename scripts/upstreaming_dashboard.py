...
def main():
    ...
    project_files = {}
    for entry in pathlib.Path("PersistentDecomp").rglob("*.lean"):
        code = None
        ...
        project_files[entry] = {
            ...
            "depends" : "import PersistentDecomp" in code
        }
    print(project_files)
    ...
    with open("./website/_includes/ready_to_upstream.md", 'w+') as writer:
        ...
        for file_path in project_files:
            ...
            text += f"* [`{module_name}`](https://github.com/Paul-Lez/PersistentDecomp/blob/master/{file_path}) \n"
    ...
    with open("./website/_includes/easy_to_unlock.md", 'w+') as writer:
        ...
        for file_path in project_files:
            ...
            module_name = file_path.replace('/','.')[:-5]
            if num_sorries == 1:
                text += f"* [`{module_name}`](https://github.com/Paul-Lez/PersistentDecomp/blob/master/{file_path}) {num_sorries} sorry\n"
            else:
                text += f"* [`{module_name}`](https://github.com/Paul-Lez/PersistentDecomp/blob/master/{file_path}) {num_sorries} sorries\n"
